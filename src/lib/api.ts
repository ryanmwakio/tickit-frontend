...
export class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      const detailedMessage = errorMessage.includes('fetch') || errorMessage.includes('Failed')
...
Then verify it's running on http://localhost:5000/api/v1`
    ...
    if (response.status === 401 && this.refreshToken && responseType !== 'blob') {
  ...
  async put<T>(endpoint: string, data?: unknown, options?: { responseType?: 'json' | 'blob' }): Promise<T> {
    return this.request<T>(endpoint, {
      method: 'PUT',
      body: data ? JSON.stringify(data) : undefined,
      ...options,
  ...
  async patch<T>(endpoint: string, data?: unknown, options?: { responseType?: 'json' | 'blob' }): Promise<T> {
    return this.request<T>(endpoint, {
      method: 'PATCH',
      body: data ? JSON.stringify(data) : undefined,
      ...options,
  ...
  async delete<T>(endpoint: string, options?: { responseType?: 'json' | 'blob' }): Promise<T> {
