...
type GroupedEventsListProps = {
  groupedEvents: {
    category: string;
    events: EventContent[];
    totalCount: number; // Total events in this category (before pagination)
    hasMore: boolean;
    onLoadMore: () => void;
...
export function GroupedEventsList({ groupedEvents }: GroupedEventsListProps) {
  ...
  return (
    <div className="space-y-12">
      {groupedEvents.map((group) => {
        ...
        const hasMore = group.hasMore;

        return (
          <div key={group.category} className="space-y-4 sm:space-y-6">
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900">{group.category}</h2>
              <div className="flex-1 border-t border-slate-200 min-w-[60px]" />
              <span className="rounded-full bg-slate-100 px-2 sm:px-3 py-1 text-xs sm:text-sm font-semibold text-slate-900 whitespace-nowrap">
                {group.totalCount} event{group.totalCount !== 1 ? "s" : ""}
              </span>
            </div>
            {/* Mobile: Horizontal Scroll, Desktop: Grid */}
            <div className="lg:grid lg:grid-cols-3 lg:gap-6 lg:gap-8">
                  ...
                  >
                    <div className="relative h-48 overflow-hidden rounded-2xl">
                      ...
                      {isPast && (
                        <div className="absolute top-3 right-3">
                          <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold ${status.className} backdrop-blur-sm`}>
                    ...
                    <div className="flex flex-1 flex-col gap-2 mt-4">
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-xs text-slate-500 truncate">{event.dateCard || event.dateFull}</p>
                        <span className="text-slate-300">•</span>
                        <p className="text-xs font-semibold text-slate-900 truncate">{event.region}</p>
                      </div>
                      <p className="flex-1 text-xs text-slate-600 line-clamp-2">{event.summary}</p>
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                        <span className={`text-sm font-bold truncate ${isPast ? 'text-slate-500' : 'text-slate-900'}`}>
                          {event.price}
                        </span>
                        <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wide ${
                          isPast 
                ...
                {visibleEvents.map((event) => {
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col gap-3 p-5">
                      ...
                      <h3 className="text-xl font-semibold text-slate-900 line-clamp-2 leading-tight">
                        {event.title}
                      </h3>
                      <div className="flex items-center gap-2">
                        <p className="text-sm text-slate-500 truncate">{event.dateCard || event.dateFull}</p>
                        <span className="text-slate-300">•</span>
                        <p className="text-sm font-semibold text-slate-900 truncate">{event.region}</p>
                      </div>
                      <p className="flex-1 text-sm text-slate-600 line-clamp-2">{event.summary}</p>
                      <div className="flex flex-wrap gap-2 mt-auto">
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                        ...
                        <span className={`inline-flex items-center gap-1 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wide ${
                          isPast 
                            ? 'bg-slate-300 text-slate-500 cursor-not-allowed' 
                            : 'bg-slate-900 text-white'
                        }`}>
            ...
            {hasMore && (
              <div className="flex justify-center pt-4 pb-2">
                ...
                >
                  {group.isLoadingMore ? (
                    <>
                      <div className="size-5 animate-spin rounded-full border-2 border-slate-300 border-t-slate-900 flex-shrink-0" />
                      <span>Loading...</span>
                    </>
                  ) : (
                    <>
                      <span>Load more events</span>
                      <ChevronDown className="size-5 transition-transform group-hover:translate-y-0.5 flex-shrink-0" />
                    </>
                  )}
                </button>
              </div>
