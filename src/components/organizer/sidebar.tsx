...
import { organizerSections } from "@/data/organizer";

const navItems = organizerSections.map((section) => ({
  id: section.id,
  label: section.title,
  href: `/organizer/${section.id}`,
}));
...
export function OrganizerSidebar() {
  ...
  const renderNavContent = (isMobile = false) => (
    <nav className={isMobile ? "p-6" : "sticky top-[calc(60px+1.5rem)] sm:top-[calc(72px+1.5rem)] lg:top-[calc(72px+2.5rem)] rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/80"}>
              ...
              >
                <div className="flex flex-col leading-tight">
                  <span className="text-[11px] font-semibold uppercase tracking-[0.35em] text-slate-400">
                    {organizerSections.find((section) => section.id === item.id)
                      ?.badge ?? "Section"}
                  </span>
                  <span className="mt-0.5 text-base font-semibold text-current">
                    {item.label}
                  </span>
                </div>
              </Link>
